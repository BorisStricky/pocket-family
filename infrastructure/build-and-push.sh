...
#                      the running service keeps the old image + schema. Idempotent:
...
# Step 4 — apply database migrations before the service picks up the new image.
...
# migration aborts the script, leaving the running service on the old image+schema.
...
# Step 5 — optionally trigger an ECS rollout so the running task picks up the
