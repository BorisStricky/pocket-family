...
## Success Criteria
- [ ] `GET /accounts` accepts optional `tenant_id` query parameter
- [ ] `AccountShareWith` schema exists in `schemas.py`
- [ ] `AccountCreate` schema has optional `share_with` field
- [ ] `POST /accounts` atomically creates account + share when `share_with` provided
- [ ] `POST /transactions` updates account balance based on transaction_type
- [ ] Backend tests pass for all new functionality
...
- [ ] Add optional `share_with` field to `AccountCreate`:
  ```python
...
- [ ] Update `create_account` endpoint to handle `share_with`:
...
- [ ] Update `create_transaction` to modify account balance:
