...
import-service/
  app/
    ...
    tasks/
      import_csv.py       # SHARED CORE: process_import() (claim + insert + DONE in ONE transaction) + _mark_import_job() — NO celery import
...
- **Idempotency claim — folded into the import transaction.** Both triggers can be handed the same message twice (Celery re-queue / Lambda retry), and a worker can die mid-import. `process_import` does the claim, the inserts, the balance update, and the final DONE flip **in ONE `get_session()` transaction**. The claim is an atomic `UPDATE importjob SET status='STARTED' WHERE id=:id AND status != 'DONE'`; if `rowcount == 0` (already DONE) it returns `{"status": "skipped"}` without inserting, so committed imports are never double-inserted. Crucially the guard is `!= 'DONE'`, **not** `= 'PENDING'`: a `STARTED` row (worker died after claiming) or a `FAILED` row (a prior attempt errored) is still re-claimable, so the redelivered message **retries** instead of being silently skipped. Because everything shares one transaction, any rollback (transient DB error, OOM/timeout/SIGKILL before commit) reverts the claim too — the job stays in a re-claimable state and SQS/Celery re-delivery re-runs it; only a retry-exhausted message lands in the DLQ. The success state (`DONE`) is committed atomically with the rows, which is what makes the `!= 'DONE'` guard safe against a concurrent, not-yet-committed re-delivery (row-level lock serializes the two claims). `FAILED` is recorded best-effort in a *separate* transaction after rollback purely for the UI; it does not block retries. When `import_job_id` is absent (local/test) there is nothing to claim and it proceeds.
